...
class A_PRO
{
     ...
     */
    function ins_usuario($DBcon, $nombre, $usuario)
    {
    ...
    public function get_listuser($DBcon,$page,$noRowsDisplay,$where)
    {
        require_once(C_P_CLASES.'utils/paginator.php');
        $now = date("Y-m-d H:i:s");

        $query = "SELECT cuser, cname, cemail, cwww, ctw, cfb  FROM 
                    tusuarios";
        $query.=$where;
    ...
    function disp_usersPage()
    {
        ...
        $encabezados = '<tr><th>Nombre Usuario</th><th>Nombre</th><th>correo</th><th>wwww</th><th></th><th>tw</th><th>Accion</th></tr>';
        ...
        while ($row = $stmt->fetchObject()) {
            ...
            $dataTable.='<tr><td>'.$row->cuser.'</td><td>'.$row->cname.'</td><td>'.$row->cemail.'</td><td>'.$row->cwwww.'</td><td>'.$row->ctw.'</td><td>'.$row->cfb.'</td>hola</tr>';
